...
#### Term 5 2023 ####
term_5_2023 <- read_sheet("https://docs.google.com/spreadsheets/d/19jxGPbdvYHPFvvNIU0-ja0svVtmqk_GCJD8tDokBOnI/edit#gid=341416317",
                          sheet = "FULL FORM",  skip = 1) %>% 
  mutate(term = 5, 
         year = 2023, 
         weeks = 8, 
...
t5_2023_unique_id <- term_5_2023  %>%
  count(mobile_number, first_name, last_name) %>%  
  mutate(unique_id = ifelse(mobile_number == "-", first_name, mobile_number)) %>% 
  select(-n)

term_5_2023_id <- term_5_2023 %>% 
  left_join(t5_2023_unique_id) 

t5_open <- term_5_2023_id %>%
  count(unique_id, open_week) %>% 
  filter(open_week == 'y') %>% 
  mutate(open_flag =1) %>% 
  select(-n, -open_week)

t5_2023_min <- term_5_2023_id %>% 
  left_join(t5_open) %>% 
  select(first_name, last_name, email, mobile_number, 
         postcode, how_did_you_hear_about_us,
         minus_transaction_fee, open_week, open_flag, total_casuals, revenue_per_casual, 
  ...
  mutate( total_upfront_dollars = total_upfront_terms * revenue_per_upfront_class, 
          total_casual_dollars = total_casuals * revenue_per_casual) %>%
  mutate(date_time_paid = str_extract(date_time_paid, "^[a-zA-Z]+ \\d{1,2}, \\d{4}"))%>%
  mutate(date_time_paid = as.character(mdy(date_time_paid)))

term_5_2023_dance_profit_raw <- read_sheet("https://docs.google.com/spreadsheets/d/19jxGPbdvYHPFvvNIU0-ja0svVtmqk_GCJD8tDokBOnI/edit#gid=341416317",
                                           sheet = "$",  skip = 12, n_max = 17, col_types = "c", col_names = FALSE) %>% 
  clean_names()
...
#### Term 6 2023 ####
term_6_2023 <- read_sheet("https://docs.google.com/spreadsheets/d/1kg3LoLJTy4qX5AjaM0pwOYumz9ThINVdABmS69t513w/edit#gid=341416317",
                          sheet = "FULL FORM",  skip = 1) %>% 
  mutate(term = 6, 
         year = 2023, 
         weeks = 8, 
...
t6_2023_unique_id <- term_6_2023  %>%
  count(mobile_number, first_name, last_name) %>%  
  mutate(unique_id = ifelse(mobile_number == "-", first_name, mobile_number)) %>% 
  select(-n)

term_6_2023_id <- term_6_2023 %>% 
  left_join(t6_2023_unique_id) 

t6_open <- term_6_2023_id %>%
  count(unique_id, open_week) %>% 
  filter(open_week == 'y') %>% 
  mutate(open_flag =1) %>% 
  select(-n, -open_week)

t6_2023_min <- term_6_2023_id %>% 
  left_join(t6_open) %>% 
  select(first_name, last_name, email, mobile_number, 
         postcode, how_did_you_hear_about_us,
         minus_transaction_fee, open_week, open_flag, total_casuals, revenue_per_casual, 
...
term_6_2023_dance_profit_raw <- read_sheet("https://docs.google.com/spreadsheets/d/19jxGPbdvYHPFvvNIU0-ja0svVtmqk_GCJD8tDokBOnI/edit#gid=341416317",
                                           sheet = "$",  skip = 12, n_max = 17, col_types = "c", col_names = FALSE) %>% 
  clean_names()
